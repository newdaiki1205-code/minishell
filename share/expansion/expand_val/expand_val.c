...
/*   Updated: 2026/04/24 16:28:19 by dshirais         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*expand_one_val(char *src, t_env *env, size_t *ex_len, int exit_status)
{
	char	*new;
	...
	if (!ft_strchr(src, '$'))
		...
		new = case_closed(src, env, ex_len, exit_status);
	else
		new = case_not_closed(src, env, ex_len, exit_status);
	if (!new)
		return (NULL);
	return (new);
}
